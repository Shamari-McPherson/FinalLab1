...
// WIFI CREDENTIALS
const char* ssid       = "MonaConnect"; // Add your Wi-Fi ssid --uwi:onaConnect home:Digicel_WiFi_A8eG
const char* password   = ""; // Add your Wi-Fi password  password:Nu5TSQ5h
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (strcmp(type, "toggle") == 0){
    ...
    doc["id"]         = "620152241"; // Change to your student ID number
    doc["timestamp"]  = getTimeStamp();
    doc["number"] = number;
    doc["ledA"] = getLEDStatus(LED_A);
    doc["ledB"] = getLEDStatus(LED_B);
